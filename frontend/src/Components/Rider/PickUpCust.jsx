  ...
  const handleSubmit = async () => {
    try {
      const data = await axios.put(
        `http://localhost:8080/api/shop/riderorders/orderDetail/orderstatus/${id}`
      )
      alert('Successfully picked up!')
      navigate(`/Rider/7`)
    } catch (err) {
      alert('cannot get data')
      console.log(err)
    ...
    <div className='pickup-cust-container'>
      ...
      <div className='pickup-cust-btn-container'>
        <button className='pickup-cust-btn' onClick={handleSubmit}>
          <Link
            to={`/Rider/RiderHomeScreen/RiderInfo/PickUpDetail/PickUpCust/ReportForm/${id}`}
          >
            Success Pick Up
          </Link>
        </button>
